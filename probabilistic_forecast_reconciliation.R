...
ggplot(long_df, aes(x = value, y = factor(year))) +
  geom_density_ridges() +
  labs(x = "dN",
       y = "Year",
...
ggplot(plot_df, aes(x = x, y = factor(year), height = density)) +
  geom_density_ridges(
    stat = "identity",
    scale = 1.5,
    alpha = 0.7
